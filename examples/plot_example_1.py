"""Example 1
   ...
   The data is processed using a Neumann the Dirichlet map.
...
g = pd.read_csv(os.path.join(meas_dir, 'BEM_results_rep_1_MSP_Neumann_data.csv'), header=None).values[:, 0]

# this is the standard deviation of the Neumann data (2nd column)
g_std = pd.read_csv(os.path.join(meas_dir, 'BEM_results_rep_1_MSP_Neumann_data.csv'), header=None).values[:, 1]


# %%
# Launch the Neumann 2 Dirichlet map
# ==================================
u, Sigma_u, A = cauchy.neumann_2_dirichlet(mesh, g, g_std)
...
# uncertainty
B, var_B = cauchy.neumann_BEM_uq(mesh, A, g, g_std, points)
